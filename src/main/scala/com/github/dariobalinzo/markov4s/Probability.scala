...
class Probability(x: Int, y: Int) {
  ...
  def add(r: Probability): Probability =
    new Probability(numer * r.denom + r.numer * denom, denom * r.denom)
...
object Probability {
  ...
  implicit def ordering[A <: Probability]: Ordering[A] =
    Ordering.by(e => e.toDouble)
}